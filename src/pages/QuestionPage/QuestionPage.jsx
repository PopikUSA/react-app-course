...
export const QuestionPage=()=>{
    ...
    const onChheckboxChangeHandler=()=>{
    ...
    <ul className={cls.cardLinks}>
        Resources:
        {
            card.resources.map((link,index)=>{
                return (
                    <li key={index}>
                        <a href={link.trim()} target='_blank' rel="norefferer">{link.trim()}</a>
                    </li>
                )
            })
        }
    ...
    <label htmlFor='checkboxId' className={cls.cardCheckbox}>
        <input type="checkbox" id={checkboxId} className={cls.checkbox} checked={isChecked} 
        onChange={onChheckboxChangeHandler} disabled={isCardUpdating}/>
